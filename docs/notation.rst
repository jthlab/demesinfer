...
In ``momi3``, all model parameters are represented as structured paths that directly reference parameters within ``demes`` graph structures. This path-based representation provides an easy way to access all demographic parameters while maintaining compatibility with the underlying ``demes`` specification.
