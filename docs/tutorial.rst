...
This tutorial demonstrates how to use the ``momi3`` module (part of the ``demestats`` package) to perform demographic inference on population models using the site frequency spectrum (SFS).

The corresponding Jupyter notebook for this tutorial is available at ``docs/momi3_tutorial.ipynb``.

We will walk through simulating population models and all the key features of momi3.  
...
Next, we simulate the ancestry of 10 diploid individuals sampled from the two subpopulations using ``msprime.sim_ancestry()``. We use a mutation and recombination rate of 1e-8 and a sequence length of 100 million base pairs, with fixed random seeds for reproducibility.
...
The output is a list of parameters, each entry representing an optimizable parameters in the IWM model. **All** of the parameters for ``momi3`` are contained in ``frozenset`` objects, and if variables are constrained to be equal by **construction** of the model, they appear grouped inside a frozenset:
...
If one is not familiar with frozenset parameters, one can also optionally use the ``variable_for`` function to take ``demes`` paths and find the associated frozenset parameter. 
...
.. code-block:: python
    ...
    demo2.add_population(name="anc", initial_size=5000)
    demo2.add_population(name="P0", initial_size=5000, growth_rate=0.002)
    demo2.add_population(name="P1", initial_size=5000, growth_rate=0.002)
    demo2.set_symmetric_migration_rate(populations=("P0", "P1"), rate=0.0001)
    demo2.add_population_split(time=1000, derived=[f"P{i}" for i in range(2)], ancestral="anc")

This is a model where P0 and P1 grow exponentially from an initial size of 5000 at a rate of 0.002 per generation. Rather than simulating, we directly examine parameters and constraints:
...
.. code-block:: python
    ...
    Log-likelihood at rate = 0.0002 is -148502.16
    Gradient at rate = 0.0002 is 3769.3643
...
The negative log-likelihood is minimized around 5250, close to the true value of 5000.
...
Here, the negative log-likelihood is minimized around 5000, close to the true value of 5000.
...
The negative log-likelihood is minimized around 900, close to the true value of 1000.
...
The negative log-likelihood is minimized close to the true value of 0.0001.

Optimization with Poisson Likelihood
-----------------------------

So far, we have used the multinomial likelihood, which is the default in sfs_loglik when we haven’t provided a mutation rate ``theta``. The multinomial likelihood conditions on the total number of segregating sites. An alternative is the Poisson likelihood, which models the absolute counts of mutations.

To use the Poisson likelihood, this requires passing **BOTH** mutation rate ``theta`` and ``sequence_length`` into the likelihood function. These parameters depend on the species and the research itself. The setup is the same as before, but now we explicitly provide these parameters. Let's try to optimize the migration rate again, but using the Poisson likelihood this time.
...
.. code-block:: python
    ...
    result = plot_sfs_likelihood(
        g, paths, vec_values,
        afs, afs_samples,
        theta=theta, # Here we provide theta and sequence_length
...
.. code-block:: python
    ...
    # Add populations
    demo3.add_population(initial_size=4000, name="anc")
    demo3.add_population(initial_size=500, name="P0", growth_rate=-np.log(3000 / 500)/66)
    demo3.add_population(initial_size=500, name="P1", growth_rate=-np.log(3000 / 500)/66)
    demo3.add_population(initial_size=100, name="P2", growth_rate=-np.log(3000 / 100)/66)

    # Set initial migration rate
    demo3.set_symmetric_migration_rate(populations=("P0", "P1"), rate=0.0001)
    demo3.set_symmetric_migration_rate(populations=("P1", "P2"), rate=0.0001)


    # population size changes near 65–66 generations
    demo3.add_population_parameters_change(
        time=65,
    ...
    demo3.add_migration_rate_change(
        time=66,
        rate=0.0005, 
        source="P0",
        dest="P1"
    ...
    demo3.add_population_split(time=5000, derived=["P0", "P1", "P2"], ancestral="anc")
